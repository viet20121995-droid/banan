...
export class InternalPdfService {
  ...
  /** Renders and returns the finished PDF bytes. */
  renderQcReport(data: QcPdfData): Promise<Buffer> {
    return this.render((doc) => {
      ...
      for (const s of data.sections) {
        const label = s.percent == null ? 'N/A' : `${s.passCount}/${s.applicable} · ${s.percent}%`;
  ...
  renderMsReport(data: MsPdfData): Promise<Buffer> {
    return this.render((doc) => {
  ...
  private render(body: (doc: Doc) => void, footerCode: string): Promise<Buffer> {
