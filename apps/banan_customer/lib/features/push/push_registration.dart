...
/// Browser glue defined in `web/index.html`. Requests notification
/// permission, registers the FCM service worker, and resolves to a token
/// (or '' if denied/unsupported).
...
Future<void> _fetchAndRegister(WidgetRef ref) async {
  ...
  try {
    ...
    // Push is optional — never surface errors to the customer.
  }
}

/// Wraps the app and registers a web-push token once per logged-in customer
/// (on launch if already signed in, and again whenever a new customer logs
/// in). Re-registering the same token is a harmless idempotent upsert.
...
class _PushRegistrarState extends ConsumerState<PushRegistrar> {
  ...
  void _maybeRegister(AuthSession? session) {
    ...
    if (user == null || !user.role.isCustomer) return;
